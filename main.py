...
async def main():
    ...
    # Запрещаем запуск с дефолтным паролем администратора
    if ADMIN_PASSWORD == "admin123":
        logger.critical(
            "ADMIN_PASSWORD использует небезопасное значение по умолчанию 'admin123'. "
            "Установите надёжный пароль через переменную окружения ADMIN_PASSWORD и перезапустите бота."
        )
        raise SystemExit(
            "Запуск с ADMIN_PASSWORD='admin123' запрещён из соображений безопасности. "
            "Задайте уникальный пароль в переменной окружения ADMIN_PASSWORD."
        )
