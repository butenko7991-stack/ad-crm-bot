...
async def autopost_view_posted(callback: CallbackQuery):
    ...
    try:
        ...
        if post.content:
            text += f"\n📝 Текст:\n{post.content[:800]}{'…' if len(post.content) > 800 else ''}\n"
...
async def _render_channel_analytics_page(message, channel_id: int, back_callback: str = "autopost_channel_analytics") -> bool:
    ...
    if telemetr_id:
        buttons.append([InlineKeyboardButton(
            ...
            url=f"https://telemetr.io/en/channels/{telemetr_id}"
        )])
...
async def _show_confirm_preview(message: Message, state: FSMContext):
    ...
    if content_text:
        preview += f"\n📝 Текст:\n{content_text[:900]}{'...' if len(content_text) > 900 else ''}\n"
    if file_id:
        preview += f"\n📎 Медиафайл: {file_type}\n"
    if buttons:
        buttons_preview = "\n".join(f"  {i+1}. [{b['text']}]({b['url']})" for i, b in enumerate(buttons))
...
async def adm_view_order(callback: CallbackQuery):
    ...
    try:
        ...
        if order.ad_content:
            text += f"\n📝 Текст:\n{order.ad_content[:800]}{'...' if len(order.ad_content) > 800 else ''}\n"
...
async def adm_view_post(callback: CallbackQuery):
    ...
    try:
        ...
        if post.content:
            text += f"📝 Текст:\n{post.content[:900]}{'...' if len(post.content) > 900 else ''}\n"
